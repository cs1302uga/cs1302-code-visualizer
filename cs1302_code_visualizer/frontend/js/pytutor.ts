  ...
  render() {
    ...
    if (this.params.embeddedMode) {
      ...
      base
        .find("#creditsPane")
        .html(
          '<div style="font-size: 8pt; margin-top: 5px; margin-bottom: 5px;"><a href="http://pythontutor.com/" target="_blank" style="color: #3D58A2;">Python Tutor</a> by <a href="http://pgbovine.net/" target="_blank" style="color: #3D58A2;">Philip Guo</a></div>',
        );
...
class DataVisualizer {
  ...
  getAllProgramVarnames() {
    ...
    $.each(this.curTrace, function (i, curEntry) {
      $.each(curEntry.ordered_globals, function (i, varname) {
        ...
        if (allVarnames.indexOf(encodedVarname) < 0) {
      ...
      $.each(curEntry.stack_to_render, function (i, frame) {
        ...
        let func_prefix = frame.func_name
          ? frame.func_name
          : DataVisualizer.UNNAMED_PREFIX;
    ...
    function traverseCStructArray(val) {
      if (val[0] == "C_STRUCT") {
        ...
        $.each(val, function (ind, kvPair) {
          if (parseInt(ind) < 3) return; // these have 3 header fields
      ...
      if (val[0] === "C_ARRAY") {
        $.each(val, function (ind, elt) {
          if (parseInt(ind) < 2) return; // these have 2 header fields
    ...
    $.each(this.curTrace, function (i, curEntry) {
      ...
      $.each(curEntry.heap, function (k, obj) {
        ...
        if (typ == "INSTANCE" || typ == "INSTANCE_PPRINT" || typ == "CLASS") {
          let [className, headerLength] =
            DataVisualizer.getClassInstanceMetadata(obj);
          for (let i = headerLength; i < obj.length; i++) {
            let fieldName = obj[i][0];
            // className can possibly be null ...
            let encodedFieldname = className
              ? className + ":" + fieldName
              : fieldName;
            if (allFieldnames.indexOf(encodedFieldname) < 0) {
              // don't insert duplicates
      ...
      function recurseIntoObject(id, curRow, newRow) {
        ...
        if (isLinearObj(heapObj)) {
          $.each(heapObj, function (ind, child) {
            if (parseInt(ind) < 1) return; // skip type tag
          ...
          let [className, headerLength] =
            DataVisualizer.getClassInstanceMetadata(heapObj);
      ...
      .data(
        function (frame) {
          if (myViz.hideVarsSet) {
            ...
            let hiddenVarnamesLst = frame.ordered_varnames.filter((varname) =>
              myViz.inHideVarsSet(func_prefix, varname),
            );
            console.log("render HIDING", func_prefix, hiddenVarnamesLst);
            hiddenVarnamesLst.forEach((e) => {
              myViz.varsHidden.push(func_prefix + ":" + e);
            });
            ...
            return frame.ordered_varnames.map(function (varname, i) {
              return {
                varname: varname,
                frame: frame,
        ...
        if (isValidPtr) {
          ...
          if (ptrVal === "0x0") {
            ...
            if (srcHeapObject.length > 0 && dstHeapObject.length > 0) {
              myViz.jsPlumbManager.heapConnectionEndpointIDs.set(
                ptrSrcId,
                ptrTargetId,
              );
            }
      ...
      if (obj.length == 1) {
        ...
        if (obj[0] == "LIST" || obj[0] == "TUPLE") {
          ...
          jQuery.each(obj, function (ind, val) {
            if (parseInt(ind) < 1) return; // skip 'SET' tag
          ...
          $.each(obj, function (ind, kvPair) {
            if (parseInt(ind) < 1) return; // skip 'DICT' tag
        ...
        if (funcProperties) {
          ...
          $.each(funcProperties, function (ind, kvPair) {
            ...
            if (myViz.inHideFieldsSet(rawFuncName, kvPair[0])) {
              ...
              myViz.fieldsHidden.push(
                (rawFuncName ? rawFuncName + ":" : "") + kvPair[0],
              );
              return; // get out!
            }
  ...
  renderCStructArray(obj, stepNum, d3DomElement) {
    ...
    if (obj[0] == "C_STRUCT") {
      ...
      if (obj.length > 3) {
        ...
        $.each(obj, function (ind, kvPair) {
          if (parseInt(ind) < 3) return; // skip header tags
      ...
      var contentTr = tbl.find("tr:last");
      $.each(obj, function (ind, val) {
        if (parseInt(ind) < 2) return; // skip 'C_ARRAY' and addr
  ...
  inHideFieldsSet(classname, fieldname) {
    return DataVisualizer._inHideSetHelper(
      this.hideFieldsSet,
      classname,
      fieldname,
    );
  ...
  static _inHideSetHelper(myMap, first, second) {
    if (!myMap) {
      ...
      // only match on second, in that case
      return myMap.has(second);
    } else {
      // match on either first:second or second
      ...
      if (lang === "js") {
        ...
        if (this.owner.params.embeddedMode) {
          ...
          this.domRoot
            .find("#langDisplayDiv")
            .html(
              'C (gcc 4.8, C11)<br/><font color="#e93f34">EXPERIMENTAL!</font> <a href="https://github.com/pgbovine/OnlinePythonTutor/blob/master/unsupported-features.md#c-and-c-unsupported-features" target="_blank">known limitations</a>',
            );
        }
      } else if (lang === "cpp") {
        if (this.owner.params.embeddedMode) {
          ...
          this.domRoot
            .find("#langDisplayDiv")
            .html(
              'C++ (gcc 4.8, C++11)<br/><font color="#e93f34">EXPERIMENTAL!</font> <a href="https://github.com/pgbovine/OnlinePythonTutor/blob/master/unsupported-features.md#c-and-c-unsupported-features" target="_blank">known limitations</a>',
            );
        }
      } else {
    ...
    uiControlsPane.find("#customizeVizLink").click(() => {
          ...
          myself.owner.dataViz.jsPlumbInstance.importDefaults({
            ...
            Overlays: [
              [
                "Arrow",
                {
                  length: uiControlsPane.find("#arrowLength").val(),
      ...
      uiControlsPane
        ...
        .change(function () {
          ...
          */
          ...
          uiControlsPane
            .find("#arrowLength,#arrowWidth,#arrowFoldback")
            .parent(".sliderWrapper")
            .show();
        ...
        .val("StateMachine")
        .change(); // <-- trigger a change event on this
      ...
      uiControlsPane.find("#updateHideVarsBtn").click(() => {
        let hideVarsLst = processHideString(
          uiControlsPane.find("#hideVars").val(),
        );
        let hideFieldsLst = processHideString(
          uiControlsPane.find("#hideFields").val(),
        );
        this.owner.dataViz.selectivelyHideVarsAndFields(
          hideVarsLst,
          hideFieldsLst,
        );
      });
