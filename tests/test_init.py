...
def test_render_image_tracer_installer_error():
    with patch("cs1302_code_visualizer.trace_generator.ensure_code_tracer_installed", side_effect=Exception("Failed")):
...
def test_render_image_trace_generation_error():
    with patch("cs1302_code_visualizer.trace_generator.generate_trace", side_effect=Exception("Trace fail")):
        with pytest.raises(Exception, match="Unable to generate execution trace!"):
            render_image(SAMPLE_JAVA)

def test_render_image_browser_driver_error():
    with patch("cs1302_code_visualizer.browser_driver.generate_image", side_effect=Exception("Render error")):
