...
from cs1302_code_visualizer.breakpoint_lister import list_breakpoints, list_breakpoints_json, main as lister_main
...
def test_list_breakpoints_tracer_error():
    ...
    with patch("cs1302_code_visualizer.trace_generator.ensure_code_tracer_installed", side_effect=Exception("Tracer fail")):
...
def test_lister_main_file_output(tmp_path, monkeypatch):
    ...
    java_home = ensure_jdk_installed()
    monkeypatch.setattr(sys, "stdin", io.StringIO(SAMPLE_JAVA))
    monkeypatch.setattr("sys.argv", ["list_breakpoints", "-o", str(out_file), "-j", "--jdk", str(java_home), "--trace-timeout", "10"])
    lister_main()
